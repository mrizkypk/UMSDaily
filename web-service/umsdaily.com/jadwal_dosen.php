...
function getHasilPencarianHTML($query) {
	$query = urlencode(trim($query));
	$url = 'https://akademik.ums.ac.id/kuliah.php?category=FNama&keyword=' . $query . '&cetak=layar&submitted=yes&mod=Jadwal_Kuliah';
...
function getHasilPencarianArray($query) {
	$jadwal = array();
	$html = getHasilPencarianHTML($query);
	if ($html == false) {
		return false;
	}
...
if (isset($_GET['nama'])) {
	if (urldecode(strtolower($_GET['nama'])) == "bana handaga") {
		echo file_get_contents('offline' . DIRECTORY_SEPARATOR . 'jadwal_dosen_bana_handaga.json');
	} else if (urldecode(strtolower($_GET['nama'])) == "nurgiyatna") {
		echo file_get_contents('offline' . DIRECTORY_SEPARATOR . 'jadwal_dosen_nurgiyatna.json');
	} else if (urldecode(strtolower($_GET['nama'])) == "heru supriyono") {
		echo file_get_contents('offline' . DIRECTORY_SEPARATOR . 'jadwal_dosen_heru_supriyono.json');
	} else if (urldecode(strtolower($_GET['nama'])) == "yogiek indra kurniawan") {
		echo file_get_contents('offline' . DIRECTORY_SEPARATOR . 'jadwal_dosen_yogiek_indra_kurniawan.json');
	} else {
		$array = getHasilPencarianArray(urldecode($_GET['nama']));
		if ($array == false) {
