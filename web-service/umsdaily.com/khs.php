...
function khsId($nim, $semester) {
  ...
  $curl->post('https://sia.ums.ac.id/get-khs-by-nim', [
    'nim' => $nim,
    'smt' => $semester
  ]);
  if ($curl->error) {
    ...
    if (strpos($response, 'KHS Tahun Akademik')) {
      foreach ($html->find('tbody tr') as $tr) {
        ...
        $pengampu = $tr->find('td', 7)->plaintext;
      ...
      foreach ($html->find('td[colspan=3]') as $key => $td) {
        if ($key == 0) {
          $khs['rangkuman']['jumlah_sks'] = $td->plaintext;
        } else if ($key == 1) {
          $khs['rangkuman']['jumlah_matakuliah'] = $td->plaintext;
        } else {
          $khs['rangkuman']['indeks_prestasi_semester'] = $td->plaintext;
        }
      }
      return $khs;
    } else {
...
if ($status == false) {
  ...
  if (strtolower($nim) == "l200144020") {
    echo file_get_contents('offline' . DIRECTORY_SEPARATOR . 'khs_' . strtolower($nim) . '_'  . $semester . '.json');
  } else {
    $array = khsId($nim, $semester);
    if ($array == false) {
